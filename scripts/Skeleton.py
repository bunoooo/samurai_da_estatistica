...
SkeletonSpriteAttackPaths = [skeleton_path + f"Skeleton Attack{i}.png" for i in range(1, 19)]


class Skeleton(pygame.sprite.Sprite):
    def __init__(self, position, moveRight, limit_left, limit_right):
        super().__init__()
        self.walkSpriteSheet = SpriteSheet(skeleton_path + "Skeleton Walk.png", SkeletonSpritesWalk)
        self.attackSprites = [pygame.image.load(path).convert_alpha() for path in SkeletonSpriteAttackPaths]
