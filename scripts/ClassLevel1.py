...
class Level1():
    ...
    },
    {
        ...
        ########################### Processa todos os layers do tipo "objectgroup"#####################
            ...
            if isinstance(layer, pytmx.TiledObjectGroup):
                for obj in layer:
                    if hasattr(obj, 'gid') and obj.gid:
                        img = self.levelData.get_tile_image_by_gid(obj.gid)
                        if img:
                            sprite = pygame.sprite.Sprite()
                            sprite.image = img
                            sprite.rect = img.get_rect(topleft=(obj.x, obj.y))
                            self.objetos.add(sprite)

        

        ########################### acaba o codigo para objeto se tiver ###############################
    ...
    def draw(self):
       ...
       # for r in self.hero.sprite.debug_platform_rects:
        ...
        for robot in self.robot:
            ...
            offset = (pos.left - robot.rect.left, pos.top - robot.rect.top)
