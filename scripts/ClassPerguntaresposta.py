...
class PerguntaResposta:
    ...
    def atualizar_opcoes(self):
        """Atualiza as opções e feedbacks com base nas dicas compradas na loja"""
    ...
    def handle_input(self):
        ...
        if keys[pygame.K_RETURN] and not self.enter_pressed_last_frame:
            ...
            self.index_resposta = self.selected_index  # <-- salva a opção escolhida
    ...
    def draw(self):
        ...
        if self.feedback_mode and self.index_resposta is not None:
          ...
          # if self.acertou is None:
           #     resultado_texto = "(Nenhuma resposta registrada)"
            if self.acertou:
                resultado_texto = "Você acertou!"
            else:
                resultado_texto = "Você errou!"
